...
corr_func=@mutual_information2;
corr_func_name='Spearman Correlation';
%@ gives other name to a function: New_name=@function
corr_func_name_calss="relieff";
feature_class_correlation=zeros(size(meas,2),1);
L=size(data_matrix,2);
%%%%%%%% corelation between feature and class:
for r=1:L %printing all the corelations (output will be after the all code)
feature_class_correlation(r)=relieff(meas(:,r),y,5);
disp([corr_func_name_calss,' of feature #',num2str(r),' = ', ...
num2str(feature_class_correlation(r))])
end
cor_feature_label=feature_class_correlation;
...
% feature_class_correlation=num2str(W');
