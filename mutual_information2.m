...
n=size(x,1); %a vector with x rows
Y=unique(y); %return the data in y but with no repetitions. The outputs is in sorted order.
values_of_x=[];
P=[];
x_ind=0;
for r=1:size(x,1)
current_x=x(r,:);
if ~is_x_in_values_of_x(current_x,values_of_x) %is_x_in_values_of_x it's a function he wrote (under this function)
x_ind=x_ind+1;
